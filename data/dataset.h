...
public:
    ...
    void make_splits(const DeviceLayout<std::vector<int>>& network_properties, const float test_share, const int partitioning_type, const int model_type);

    TrainSet* get_train_set(const int& nid, const int& did);
    TestSet* get_test_set(const int& nid, const int& did);
    std::unordered_map<int, std::unordered_map<int, int>>* get_mapping(const int& nid, const int& did);

private:
    void make_partitions(const DeviceLayout<std::vector<int>>& network_properties, const float test_share, const int partitioning_type, const int model_type);
    ...
    std::pair<int,int> get_smallest_train(const DeviceLayout<TrainSet>& training_queries);
    std::pair<int,int> get_smallest_relative_train(const DeviceLayout<TrainSet>& training_queries, const DeviceLayout<std::vector<int>>& network_properties);
    std::pair<int,int> get_smallest_arch_train(const DeviceLayout<TrainSet>& training_queries, const DeviceLayout<std::vector<int>>& network_properties);
    ...
    DeviceLayout<std::unordered_map<int, std::unordered_map<int, int>>> qd_parameters; // Node ID -> Device ID -> Query ID -> Document ID -> QD-pair index.
    // A multi-dimensional array imitating the layout of the nodes and devices in the network.
    // Each node's device array contains the number of query-document pairs represented by the qd_parameters mapping.
    DeviceLayout<int> qd_parameters_sz; // Node ID -> Device ID -> Number of QD-pairs.
    // A multi-dimensional array imitating the layout of the nodes and devices in the network.
    // Each node's device array is a 1D array of contiguous query sessions with the same query ID.
    DeviceLayout<TrainSet> training_queries; // Node ID -> Device ID -> SERP_Hst's grouped by query.
    // A multi-dimensional array imitating the layout of the nodes and devices in the network.
    ...
    DeviceLayout<TestSet> testing_queries; // Node ID -> Device ID -> SERP_Hst's grouped by session.

    // Utility function used to reshape a multi-dimensional array to the same
    // dimensions as another array.
    template<typename A, typename B>
    void init_network(DeviceLayout<A>& src, const DeviceLayout<B>& dst) {
        src.resize(dst.size());
