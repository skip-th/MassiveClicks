...
#ifndef CLICK_MODEL_TMP_H
#define CLICK_MODEL_TMP_H
...
class TMP_Host: public ClickModel_Host {
public:
    HST TMP_Host();
    HST TMP_Host(TMP_Host const &tmp);
    HST TMP_Host* clone() override;
...
private:
    ...
    HST void init_example_parameters(const std::tuple<std::vector<SERP>, std::vector<SERP>, int>& partition, const int n_devices);
    ...
    int n_exams_dev{0}; // Size of the device-side examination parameters.
    int n_tmp_exams_dev{0}; // Size of the device-side temporary examination parameters.

    std::vector<Param> example_parameters; // Host-side example parameters.
    std::vector<Param> tmp_example_parameters; // Host-side temporary example parameters.
    Param* example_param_dptr; // Pointer to the device-side example parameters.
    Param* tmp_example_param_dptr; // Pointer to the device-side temporary example parameters.
    int n_example_dev{0}; // Size of the device-side example parameters.
    int n_tmp_example_dev{0}; // Size of the device-side temporary example parameters.
...
class TMP_Dev: public ClickModel_Dev {
public:
    DEV TMP_Dev();
    DEV TMP_Dev(TMP_Dev const &tmp);
...
private:
    ...
    Param* example_parameters;
    Param* tmp_example_parameters;
    int n_example_parameters{0};
    int n_tmp_example_parameters{0};
};

#endif // CLICK_MODEL_TMP_H