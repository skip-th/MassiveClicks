/** Template click model class.
 * Pooya Khandel's ParClick is used as a reference implementation.
 *
 * tmp.cu:
 *  - Defines the functions specific to creating a TMP CM.
 */
...
/**
 * @brief Allocate device-side memory for the example parameters.
 *
 * @param partition The training and testing sets, and the number of
 * query-document pairs in the training set.
 * @param n_devices The number of devices on this node.
...
HST void TMP_Host::init_example_parameters(const std::tuple<std::vector<SERP>, std::vector<SERP>, int>& partition, int n_devices) {
    ...
    // Allocate memory for the example parameters on the device.
    this->n_example_dev = MAX_SERP_LENGTH; // Set the number of example parameters.
    this->example_parameters.resize(this->n_example_dev, default_parameter);
    CUDA_CHECK(cudaMalloc(&this->example_param_dptr, this->n_example_dev * sizeof(Param)));
    CUDA_CHECK(cudaMemcpy(this->example_param_dptr, this->examination_parameters.data(),
                          this->n_example_dev * sizeof(Param), cudaMemcpyHostToDevice));

    // Allocate memory for the temporary example parameters on the device.
    // These values are replaced at the start of each iteration, which means
    // they don't need to be initialized with a CUDA memory copy.
    this->n_tmp_exams_dev = std::get<0>(partition).size() * this->n_example_dev; // Set the number of example parameters required for temporary thread result storage.
    this->tmp_examination_parameters.resize(this->n_tmp_exams_dev);
    CUDA_CHECK(cudaMalloc(&this->tmp_exam_param_dptr, this->n_tmp_exams_dev * sizeof(Param)));
...
HST void TMP_Host::get_device_references(Param**& param_refs, int*& param_sizes) {
    ...
    tmp_param_refs_array[4] = this->example_param_dptr;
    tmp_param_refs_array[5] = this->tmp_example_param_dptr;
    ...
    tmp_param_sizes_array[4] = this->n_example_dev;
    tmp_param_sizes_array[5] = this->n_tmp_example_dev;
...
HST void TMP_Host::reset_parameters(void) {
    ...
    std::vector<Param> cleared_example_parameters(this->n_attr_dev, default_parameter);
    ...
    CUDA_CHECK(cudaMemcpy(this->example_param_dptr, cleared_example_parameters.data(), this->n_example_dev * sizeof(Param), cudaMemcpyHostToDevice));
}
...
HST void TMP_Host::transfer_parameters(int parameter_type, int transfer_direction) {
    ...
    if (parameter_type == PUBLIC || parameter_type == ALL) {
        ...
        if (transfer_direction == D2H) { // Transfer from device to host.
            // Retrieve the example parameters from the device.
            CUDA_CHECK(cudaMemcpy(this->examination_parameters.data(), this->exam_param_dptr, this->n_exams_dev * sizeof(Param), cudaMemcpyDeviceToHost));
        }
        else if (transfer_direction == H2D) { // Transfer from host to device.
            // Send the example parameters to the device.
            CUDA_CHECK(cudaMemcpy(this->exam_param_dptr, this->examination_parameters.data(), this->n_exams_dev * sizeof(Param), cudaMemcpyHostToDevice));
        }
    }
...
HST void TMP_Host::get_parameters(std::vector<std::vector<Param>>& destination, int parameter_type) {
    ...
    if (parameter_type == PUBLIC) {
        ...
        destination[1] = this->example_parameters;
    }
        ...
        destination[2] = this->example_parameters;
    }
}

/**
 * @brief Compute the result of combining the TMP parameters from other nodes
 * or devices.
 *
...
HST void TMP_Host::set_parameters(std::vector<std::vector<Param>>& source, int parameter_type) {
    ...
    if (parameter_type == PUBLIC) {
        ...
        this->example_parameters = source[1];
    }
    else if (parameter_type == PRIVATE) {
        ...
        this->example_parameters = source[2];
    }
}

/**
 * @brief Compute the log-likelihood of the current TMP for the given query
 * session.
 *
...
/**
 * @brief Compute the click probability of the current TMP for the given query
 * session.
 *
...
HST void TMP_Host::destroy_parameters(void) {
    ...
    // Free origin and temporary examination containers.
    CUDA_CHECK(cudaFree(this->example_param_dptr));
    CUDA_CHECK(cudaFree(this->tmp_example_param_dptr));
...
//---------------------------------------------------------------------------//
// Device-side TMP click model functions.                                    //
//---------------------------------------------------------------------------//

/**
 * @brief Prints a message.
 */
DEV void TMP_Dev::say_hello() {
    printf("Device-side TMP says hello!\n");
}

/**
 * @brief Creates a new TMP click model object.
 *
 * @return TMP_Dev* The TMP click model object.
 */
DEV TMP_Dev *TMP_Dev::clone() {
    return new TMP_Dev(*this);
}

DEV TMP_Dev::TMP_Dev() = default;

/**
 * @brief Constructs a TMP click model object for the device.
 *
 * @param tmp
 * @returns TMP_Dev The TMP click model object.
 */
DEV TMP_Dev::TMP_Dev(TMP_Dev const &tmp) {
}
 ...
 */
DEV void TMP_Dev::set_parameters(Param**& parameter_ptr, int* parameter_sizes) {
    ...
    this->example_parameters = parameter_ptr[4];
    this->tmp_example_parameters = parameter_ptr[5];
    ...
    this->n_example_parameters = parameter_sizes[4];
    this->n_tmp_example_parameters = parameter_sizes[5];
}

/**
 * @brief Compute a single Expectation-Maximization iteration for the TMP click
 ...
 * @param partition_size The size of the dataset.
 */
DEV void TMP_Dev::update_examination_parameters(SERP& query_session, int& thread_index, int& block_index, int& partition_size) {
...
/**
 * @brief Update the global example parameters using the local
 * example parameters of a single thread.
 *
 * @param query_session The query session of this thread.
 * @param thread_index The index of this thread.
 */
DEV void TMP_Dev::update_example_parameters(SERP& query_session, int& thread_index) {
    // TODO
}
