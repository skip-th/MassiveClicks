...
namespace Communicate
{
     ...
     */
    void send_partitions(const int& node_id, const int& n_nodes, const int& n_devices, const int& total_n_devices, const int* n_devices_network, Dataset& dataset, std::vector<std::tuple<std::vector<SERP>, std::vector<SERP>, int>>& device_partitions, std::vector<std::unordered_map<int, std::unordered_map<int, int>>*>& root_mapping) {
        // Create SERP MPI datatype.
        MPI_Datatype MPI_SERP;
        MPI_CHECK(MPI_Type_contiguous(sizeof(SERP) / sizeof(int), MPI_INT, &MPI_SERP));
